...
#define	MSG_DATA_header  (sizeof(struct msg_data) - MSG_DATA_size)
#define MSG_DATA_sendLEN(_msgdata)  (MSG_DATA_header + strlen(_msgdata.data))
...
ssize_t Msgrcv(const int &msqid,
			   ...
			   const long &msgtyp,
			   const int &msgflg,
			   const uint8_t mode=1); // 从消息队列接收消息
