...
/** 注意一个坑：消息队列的名称只能有开头一个 / 符号！！！！
	...
	2.attr.mq_maxmsg 不能超过 /proc/sys/fs/mqueue/msg_max 的数值。
	3.attr.mq_msgsize不能超过 /proc/sys/fs/mqueue/msgsize_max 的数值。  */
mqd_t Mq_open(const char *pathname, int oflag, ... /* 可直接调用 */ ) // 打开/创建 消息队列
{
	mqd_t mqd;
	if (oflag & O_CREAT) {
		mode_t mode;
		...
		mqd = mq_open(pathname, oflag, mode, attr);
		va_end(ap);
	}
	else {
		mqd = mq_open(pathname, oflag);
	}
	if (mqd == mqd_t(-1)) {
...
void Mq_close(const mqd_t &mqd)
{
	if (mq_close(mqd) == -1)
		err_sys(-1, __FILE__, __func__, __LINE__, "mq_close()");
}

void Mq_unlink(const char *pathname)
{
	if (mq_unlink(pathname) == -1)
		err_ret(-1, __FILE__, __func__, __LINE__, "mq_unlink()");
}

void Mq_send(const mqd_t &mqd, const char *ptr, const size_t &len, const uint &prio)
{
	if (mq_send(mqd, ptr, len, prio) == -1)
		err_sys(-1, __FILE__, __func__, __LINE__, "mq_send()");
}

ssize_t Mq_receive(const mqd_t &mqd, char *ptr, const size_t &len, uint *prio,
				   const uint8_t &mode)
{
	memset(ptr, 0, len);
	ssize_t sizeval;
	if ((sizeval = mq_receive(mqd, ptr, len, prio)) < 0) {
		if (errno == EAGAIN/* O_NONBLOCK : Try again */ ) return -1234;
		err_sys(sizeval, __FILE__, __func__, __LINE__, "mq_receive()");
	}
	if (mode == 1 && sizeval > 1) {
		ptr = ptr + sizeval - 1;
...
void Mq_notify(const mqd_t &mqd, const struct sigevent *notification)
{
	if (mq_notify(mqd, notification) == -1)
		err_sys(-1, __FILE__, __func__, __LINE__, "mq_notify()");
}

void Mq_getattr(const mqd_t &mqd, struct mq_attr *mqstat)
{
	bzero(mqstat, sizeof(struct mq_attr));
	if (mq_getattr(mqd, mqstat) == -1)
		err_sys(-1, __FILE__, __func__, __LINE__, "mq_getattr()");
}

void Mq_setattr(const mqd_t &mqd, const struct mq_attr *mqstat,
				struct mq_attr *omqstat)
{
	if (mq_setattr(mqd, mqstat, omqstat) == -1)
		err_sys(-1, __FILE__, __func__, __LINE__, "Mq_setattr()");
}
